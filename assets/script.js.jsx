    ...
    render: function() {
        <li>
            <label>
                ...
                <span>{this.props.topicLetter + ". "}</span>
            </label>
            <div className="topic">
...
var ElementContainer = React.createClass({
    ...
    listTopics: function() {
        let propRefs = Object.keys(this.props.topic);
        console.log(propRefs);
        let output = (() => {
            for (let prop = 0; prop < propRefs.length; prop++) {
                return <TopicBox
                    topicLetter={propRefs[prop]}
                    topicContent={eval('this.props.title.' + propRefs[prop]) + '.text'}/>;
            };
        });
        console.log(output);
        return output;
